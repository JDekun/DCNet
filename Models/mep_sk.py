...
class DeepLabV3(nn.Module):
    ...
    def forward(self, x: Tensor, target=None, is_eval = False) -> Dict[str, Tensor]:
        ...
        if  (self.contrast ) and (is_eval == False):
            ...
            aspp_one = F.normalize(temp, dim=1)
...
class SkHead(nn.Sequential):
    def __init__(self, channel: int, num_classes: int, contrast, attention) -> None:
        super(SkHead, self).__init__(
            SKAttention(channel_in=channel,channel=512,reduction=8),
...
def mep_sk(args, aux, num_classes=21, pretrain_backbone=False):
    ...
    classifier = SkHead(out_inplanes, num_classes, args.contrast, args.attention)
